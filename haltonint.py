...
def halton_integrator(f, low, high, N=100, seed=110):
    ...
    for n in range(1, N): # calculate unscaled estimates for 1 to N samples dynamically
...
def halton_plot(f, f_label="$f(x)$", low=0, high=1, N=100, filename="tmp"):
    ...
    #ax.set_title("Integral estimates")
    fig.savefig(f"figures/{filename}-integrals.png")
    plt.show()
    ...
    if d == 1:
        ...
        fig.savefig(f"figures/{filename}-graph.png")
        plt.show()
    if d == 2:
        ...
        ax.view_init(18, 166)
        fig.savefig(f"figures/{filename}-graph.png", bbox_inches='tight')
        plt.show()

# d-dimensional sampling and integral over cube (low, high)^d
